...
def update_formula(version, sha256=None):
    ...
    if sha256:
        formula_content, replacements = re.subn(
            r'((?:url\s+"https://github\.com/nvk/webdownloader/archive/refs/tags/v[\d.]+\.tar\.gz"\n\s*)sha256\s+")([a-f0-9]+)(")',
            rf'\g<1>{sha256}\g<3>',
            formula_content,
            count=1
        ...
        if replacements != 1:
            raise ValueError("Could not find the top-level release SHA256 in the formula")
...
def main():
    ...
    print("5. Update the nvk/homebrew-tap formula")
    print("6. Copy the updated formula to nvk/homebrew-tap/Formula/webdownloader.rb")
